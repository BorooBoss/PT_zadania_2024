...
void fn0(int first, int second){
    cout << first << " " << second << endl; //end line
}
void fn1(int first = 2, int second = 5){ //implicitne parametre, fn1() nastavi 2,5. fn1(1,1) nastavi 1,1;
    cout << first << " " << second << endl; //end line
}
...
public:
    ...
    explicit String(const char *str2) {
        if (str2){
            size_t length = std::strlen(str2);
            data = new char[length + 1];
            std::strcpy(data, str2);
        }
        else{
            data = new char[1];
            data[0] = '\0';
    ...
    void append(const char *text){
        ...
        size_t new_length = data_length + text_length;
