...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    auto it = std::find(data.cbegin(), data.cend(), value);
...
list<int>::const_iterator findInRange200to400(const list<int> & data) noexcept {
    return std::find_if(data.cbegin(), data.cend(), isInRange200to400);
}
...
list<int> add(const list<int> & data1, const list<int> & data2) noexcept {
    std::list<int> output(data1.size());
    std::transform(data1.cbegin(), data1.cend(), data2.cbegin(), output.begin(), std::plus<int>());
...
class T{
    vector<int> v;
public:
    T(const initializer_list<int>& int_list){
        for(const int e : int_list){
            v.push_back(e);
        }
    }
};
int main() {
    ...
    /*
    int num1{}; //defaultne inicializujem pomocou {} na 0;
    int num2{10}; //nastavim na 10;
    S s1{};
    S s2{5};
    S s3[]{{5},{4},{7}};
    int pole[]{4,4,4,4,4,4,4,4,4,};
    int pol2[4]{2,2}; //2,2,0,0 -> cleny

    C c1{};
    C c2{2};
    C* dyn_pole = new C[3]{1,1,1}; //C[]alokujem nahodne{}rovno aj nastavim hodnoty
*/
    return 0;
}
