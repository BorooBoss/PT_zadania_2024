...
void reverse(queue<int> & data) noexcept {
    std::stack<int> stack; //vytvor pomocny stack
    while (!data.empty()){
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
    ...
    for (size_t l = 0; l < size1; ++l){
        std::vector<int> innerVector(size2,value); //vytvor vnoreny s roymerzmi
        vec.push_back(innerVector); //do hlavneho dam vnoreny
    }
...
int main() {
    ...
    string f = "jozef";
    int op = 10;
    string result = connectWithSpace(f,op);
    cout << result;
     */
    return 0;
}
